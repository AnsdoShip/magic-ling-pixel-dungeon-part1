...
public class RedDragonRing extends Artifact {

    {
        image = ItemSpriteSheet.GRRENSHILED;
        ...
        if (ghost == null){
            if (charge < chargeCap) {
                ...
                if (charge >= chargeCap) {
                    charge = chargeCap;
                    partialCharge = 0;
                    GLog.p(Messages.get(RedDragonRing.class, "charged"));
                }
                updateQuickslot();
            }
        } else {
    ...
    public static class Petal extends Item {
        ...
        public boolean doPickUp( Hero hero ) {
            RedDragonRing rose = hero.belongings.getItem( RedDragonRing.class );
    ...
    public static class RedKing extends NPC {
        ...
        private void updateRose(){
            if (rose == null) {
                rose = Dungeon.hero.belongings.getItem(RedDragonRing.class);
            }
